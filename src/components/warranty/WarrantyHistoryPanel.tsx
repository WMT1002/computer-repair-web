          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 text-xs font-mono font-bold rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                  即時序號履歷庫
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredList.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="bg-slate-800/90 border border-slate-700/80 rounded-xl p-4 shadow-lg hover:border-cyan-500/50 transition-all flex flex-col justify-between group"
              >
                {/* Top Header */}
                <div>
                  ...
                  <div className="flex items-start gap-2.5 mb-2">
                    <div className="w-8 h-8 rounded-lg bg-cyan-500/10 text-cyan-400 flex items-center justify-center border border-cyan-500/30 shrink-0 mt-0.5">
                      <Icon className="w-4 h-4" />
                    </div>
                    <div>
                      <h3 className="text-sm font-bold text-slate-100 group-hover:text-cyan-300 transition line-clamp-2">
                        {item.partName}
                      </h3>
                      {item.supplier && (
                  ...
                  <div className="bg-slate-900/80 p-2.5 rounded-lg border border-slate-700/60 my-2.5 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-1 rounded text-slate-400 hover:text-cyan-300 hover:bg-slate-800 transition cursor-pointer shrink-0"
                      title="複製序號"
                    >
                      {copiedId === item.id ? (
                  ...
                  {/* If not picked up */}
                  {!item.isPickedUp && (
                    <div className="mb-3 p-2 rounded-lg bg-amber-950/20 border border-amber-500/20 text-[11px] text-amber-300/90 flex items-center justify-between font-mono">
                      <span>⏳ 顧客尚未取件</span>
                      {item.repair && onTogglePickedUp && (
                        <button
                          onClick={() => onTogglePickedUp(item.customerId, item.repairId, true)}
                          className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 hover:bg-emerald-500/30 text-[10px] font-bold transition cursor-pointer"
                        >
                          立即打勾已取件
                        </button>
                      )}
                  ...
                  <div className="space-y-1 text-xs text-slate-400 font-mono pt-2 border-t border-slate-700/50">
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1">
                        <User className="w-3.5 h-3.5 text-slate-500" />
                        <strong className="text-slate-200">{item.customerName}</strong>
                      </span>
                      <span>{item.customerPhone}</span>
                    </div>

                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-slate-500">工單單號：</span>
                ...
                {/* Card Bottom Actions */}
                <div className="flex items-center justify-between pt-3 mt-3 border-t border-slate-700/50 gap-2">
                  <button
                    onClick={() => setPrintingRecord(item)}
                    className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-slate-700/70 text-slate-200 hover:bg-cyan-500/20 hover:text-cyan-300 hover:border-cyan-500/30 border border-slate-600 flex items-center gap-1 transition cursor-pointer"
                    title="列印零件保固卡憑證"
                  >
        ...
        <div className="bg-slate-800/90 border border-slate-700/80 rounded-xl overflow-hidden shadow-xl">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs font-mono">
              <thead className="bg-slate-900/90 border-b border-slate-700 text-slate-400 font-bold uppercase tracking-wider">
                <tr>
                ...
                {filteredList.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-750/50 transition">
                    ...
                    <td className="py-3 px-4 text-slate-300 whitespace-nowrap text-[11px]">
                      <div>{item.warrantyDays} 天</div>
                      <div className="text-slate-500">訖: {item.endDateStr}</div>
                    </td>
                    ...
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() => setPrintingRecord(item)}
                          className="p-1.5 rounded text-slate-400 hover:text-cyan-300 hover:bg-slate-700"
                          title="列印保固證明"
                        >
                          <Printer className="w-3.5 h-3.5" />
                        </button>
                        <button
                          ...
                          className="p-1.5 rounded text-slate-400 hover:text-amber-300 hover:bg-slate-700"
                          title="編輯"
                        >
                          <Edit3 className="w-3.5 h-3.5" />
                        </button>
